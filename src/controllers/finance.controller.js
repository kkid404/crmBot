...
async function exportFinanceReport(month, year) {
  ...
  tasks.forEach(t => {
    ...
    byCreator.set(creatorKey, { count:(byCreator.get(creatorKey)?.count||0)+1, sum:(byCreator.get(creatorKey)?.sum||0)+bonus });
    byBuyer.set(buyerKey,     { count:(byBuyer.get(buyerKey)?.count||0)+1,     sum:(byBuyer.get(buyerKey)?.sum||0)+bonus });
  });

  // --------------- 3. build sheet‑data arrays
  const creatorBonusRows = [['Креативщик','К‑во креативов','Сумма бонусов']];
  for (const [name,{count,sum}] of byCreator) creatorBonusRows.push([name,count,sum]);

  const buyerBonusRows = [['Баер','К‑во креативов','Сумма бонусов']];
  for (const [name,{count,sum}] of byBuyer) buyerBonusRows.push([name,count,sum]);

  const creativesByBuyerRows = [['Баер','Название ТЗ', 'Бонус' ,'Дата']];
  tasks.forEach(t => {
    creativesByBuyerRows.push([
      safeUsername(t.buyer),
      t.name,
      t.bonus ?? 0,                       // ← новая колонка
      t.completionDate ? new Date(t.completionDate).toLocaleDateString('ru-RU') : ''
    ]);
  });

  const creativesByCreatorRows = [['Креативщик','Название ТЗ', 'Бонус' ,'Дата']];
  tasks.forEach(t => {
