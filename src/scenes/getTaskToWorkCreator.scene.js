...
getTTScene.enter(async (ctx) => {
    try {
        ...
        if (!user) {
            await ctx.reply(ruMessage.messages.user_not_found);
        ...
        if (!task) {
            await ctx.reply('Нет доступных задач для автоматического выбора');
            return ctx.scene.leave();
        }
...
📅 Дата создания: ${task.createdAt.toLocaleDateString()}
        ...
        // Переходим сразу к выбору даты
    } catch (error) {
        console.error('Ошибка при автоматическом выборе задачи:', error);
        await ctx.reply('Произошла ошибка при автоматическом выборе задачи');
        ctx.scene.leave();
    }
});
...
// Удален обработчик выбора задачи по ID, так как теперь используется только автоматическое назначение
