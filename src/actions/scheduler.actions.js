        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.links.taskSheets && result.links.taskSheets.length > 0) {
                    ...
                    const show = name => {
                        const item = result.links.taskSheets.find(s => s.name === name);
                        if (item) linksMessage += `• [${item.name}](${item.url})\n`;
                    };

                    show('Креативы в работе');
                    show('Выполненные креативы');
                    show('Все креативы');
                    show('Выполненные креативы new');
                }
